...
for(d in 1:8){
  ...
  for(x in 1:1){
    ...
    trip_data <-trip_data %>%
      dplyr::mutate(
        vA=beta_sqrt_cod_keep*sqrt(tot_keep_cod_base) +
          ...
          beta_sqrt_cod_hadd_keep*(sqrt(tot_keep_cod_base)*sqrt(tot_keep_hadd)) +
          beta_cost*cost_base, 
    ...
    write_xlsx(trip_data,paste0("trip_data_check", cop_name, "_", ind_or_corr, "_", d,".xlsx")) 
    ...
    mean_trip_data<- mean_trip_data %>% 
      dplyr::mutate(tot_cod_cat=tot_keep_cod_base+tot_rel_cod_base, 
                    ...
                    tot_keep_cod=tot_keep_cod_base, 
                    tot_rel_cod=tot_rel_cod_base)
    
    mean(mean_trip_data$tot_rel_cod)
    mean(mean_trip_data$tot_rel_cod_base)
    mean(mean_trip_data$tot_keep_cod)
    mean(mean_trip_data$tot_keep_cod_base)
    
    
    mean(mean_trip_data$tot_rel_hadd)
    mean(mean_trip_data$tot_rel_hadd_base)
    mean(mean_trip_data$tot_keep_hadd)
    mean(mean_trip_data$tot_keep_hadd_base)
    ...
    mean_trip_data<-mean_trip_data %>% 
      dplyr::mutate(tot_keep_cod_unw=tot_keep_cod, 
                    tot_keep_hadd_unw=tot_keep_hadd, 
                    tot_rel_cod_unw=tot_rel_cod, 
                    tot_rel_hadd_unw=tot_rel_hadd, 
                    tot_cod_cat_unw=tot_cod_cat, 
                    tot_hadd_cat_unw=tot_hadd_cat, 
                    tot_keep_cod_base_unw=tot_keep_cod_base, 
                    tot_keep_hadd_base_unw=tot_keep_hadd_base, 
                    tot_rel_cod_base_unw=tot_rel_cod_base, 
                    tot_rel_hadd_base_unw=tot_rel_hadd_base, 
                    tot_cod_cat_base_unw=tot_cod_cat_base, 
                    tot_hadd_cat_base_unw=tot_hadd_cat_base)
    ...
    write_xlsx(calibration_data_all,paste0("calibration_data_all_test2_check", cop_name, "_", ind_or_corr, "_", d,".xlsx")) 
    ...
    trip_level_output <- trip_level_output %>%
      ...
      .[, cod_keep_sum_unw_base := expand*tot_keep_cod_base_unw] %>%
      .[, hadd_keep_sum_unw_base := expand*tot_keep_hadd_base_unw] %>%
      .[, cod_rel_sum_unw_base := expand*tot_rel_cod_base_unw] %>%
      .[, hadd_rel_sum_unw_base := expand*tot_rel_hadd_base_unw] %>%
      .[, cod_catch_sum_unw_base := expand*tot_cod_cat_base_unw] %>%
      .[, hadd_catch_sum_unw_base := expand*tot_hadd_cat_base_unw] %>%
      
      .[, cod_keep_sum_unw := expand*tot_keep_cod_unw] %>%
      .[, hadd_keep_sum_unw := expand*tot_keep_hadd_unw] %>%
      .[, cod_rel_sum_unw := expand*tot_rel_cod_unw] %>%
      .[, hadd_rel_sum_unw := expand*tot_rel_hadd_unw] %>%
      .[, cod_catch_sum_unw := expand*tot_cod_cat_unw] %>%
      .[, hadd_catch_sum_unw := expand*tot_hadd_cat_unw] %>%
      
      .[, ntrips_base := expand*prob0]
    
    sum(trip_level_output$cv_sum)
    trip_level_output <- trip_level_output %>%
      mutate_if(is.numeric, replace_na, replace = 0)    
