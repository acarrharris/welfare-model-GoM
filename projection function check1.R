...
catch_data_all <- catch_data_all %>% dplyr::mutate(decade=1)
...
#     
    ...
    catch_data_cod<- catch_data_cod %>% dplyr::select(fishid, decade, tripid, keep_tot, release, period2, catch_draw) 
    catch_data_cod<- catch_data_cod %>% 
    ...
    summed_catch_data <- catch_data_cod %>%
      as.data.table() %>%
      .[,lapply(.SD, sum), by =c("period2", "catch_draw", "tripid", "decade"), .SDcols = c("keep", "release")]
    ...
    if (hadd_catch_check!=0){
      ...
      hadd_catch_data<- subset(hadd_catch_data, select=c(fishid, decade, tripid, keep_adj, release, period2, catch_draw)) %>% 
      ...
      trip_data <- trip_data %>% 
        dplyr::left_join(trip_data_hadd, by = c("period2", "catch_draw", "tripid",  "decade")) 
    ...
    predictions01<- as.data.frame(cbind(
      cv_i,
      cod_keep_i,
      hadd_keep_i,
      cod_rel_i,
