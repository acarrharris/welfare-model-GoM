---
title: "Nature Climate Change paper results"
...
pkgs_to_use <- c("tidyverse", "data.table", "readxl", "haven", "stringr", "knitr",
                 ...
                 "writexl", "sjPlot", "tictoc", "Hmisc", "plyr", "dplyr", "conflicted", "gt")
...
# Load data
df<-data.frame(read_csv("E:/Lou_projects/welfare-model-GoM/input_data/directed trips and regulations 2010_2020_disaggregated.csv"), show_col_types = FALSE)
...
## Welfare loss from full closures, by year {.tabset}
```{r echo = FALSE,  fig.width=10}
# close cod and haddock
...
close_both<-rbind.fill(close_cod19,close_cod20,close_cod21) 

# close cod
yr<-2019
close_cod19<-read_excel(paste0(output_data_cd,"model_output_y", yr, "_outliers_9-21-25_close cod.xlsx")) %>% 
  dplyr::rename(cv=change_CS) %>% 
  dplyr::group_by(draw)  %>% 
  summarise(across(c(cv, tot_keep_cod_new, tot_keep_hadd_new), \(x) sum(x, na.rm = TRUE)), .groups="drop") %>% 
  dplyr::ungroup()  %>% 
...
yr<-2020
close_cod20<-read_excel(paste0(output_data_cd,"model_output_y", yr, "_outliers_9-21-25_close cod.xlsx")) %>% 
  dplyr::rename(cv=change_CS) %>% 
  dplyr::group_by(draw)  %>% 
  summarise(across(c(cv, tot_keep_cod_new, tot_keep_hadd_new), \(x) sum(x, na.rm = TRUE)), .groups="drop") %>% 
  dplyr::ungroup()  %>% 
...
yr<-2021
close_cod21<-read_excel(paste0(output_data_cd,"model_output_y", yr, "_outliers_9-21-25_close cod.xlsx")) %>% 
  dplyr::rename(cv=change_CS) %>% 
  dplyr::group_by(draw)  %>% 
  summarise(across(c(cv, tot_keep_cod_new, tot_keep_hadd_new), \(x) sum(x, na.rm = TRUE)), .groups="drop") %>% 
  dplyr::ungroup()  %>% 
...
gt::gt(close_cod) %>%
  gt::tab_header(
    title = "Total CV from closing cod, by year",
  ) %>%
  gt::fmt_number(
    columns = c(mean_cv, sd_cv),   # numeric columns to format
    decimals = 0,                  # number of decimal places
    use_seps = TRUE                # adds commas as thousands separators
  )
``` 

### Cod and haddock closure
```{r echo = FALSE,  fig.width=10}
gt::gt(close_both) %>%
  gt::tab_header(
    title = "Total CV from closing both fisheries, by year",
  ) %>%
  gt::fmt_number(
    columns = c(mean_cv, sd_cv),   # numeric columns to format
    decimals = 0,                  # number of decimal places
...
Each tab contains different projected fishery outcomes across copula models for the case of dependence, independence, and the difference between dependence and independence. Projected catch-per-trip data from 2025-09-18. 
```{r echo = FALSE,  fig.width=10}
...
for(yr in years){
  ...
  ktau_data<-read_excel(paste0(output_data_cd,"ktau_output_y", yr, "_outliers_10-8-25.xlsx"))
  ktau_annual<-ktau_data %>% filter(month==0 & domain1=="all") %>% 
    dplyr::select(-k_tau_catch_p, -k_tau_keep_p, -domain1) %>% 
    dplyr::mutate(pct_both_above10=(both_above_10/2000)*100,
                  pct_both_below10=(both_below_10/2000)*100,
                  pct_both_above25=(both_above_25/2000)*100,
                  pct_both_below25=(both_below_25/2000)*100,
                  pct_both_above50=(both_above_50/2000)*100,
                  pct_both_below50=(both_below_50/2000)*100,
                  pct_both_above75=(both_above_75/2000)*100,
                  pct_both_below75=(both_below_75/2000)*100,
                  pct_both_above90=(both_above_90/2000)*100,
                  pct_both_below90=(both_below_90/2000)*100)
  ...
  ktau_annual_long <- ktau_annual %>%
    ...
    select(draw, correlation, copula, decade, k_tau_keep_est, k_tau_catch_est,
                dplyr::matches("^pct_.*_(below|above)(10|25|50|75|90)$"), 
                dplyr::matches("^both_(below|above)_"))
  
  ktau_data_ind<-read_excel(paste0(output_data_cd,"ktau_output_y", yr,"_outliers_10-8-25_ind.xlsx"))
  ...
  #plot data 
  output_summarized1<- read_rds(paste0(output_data_cd,"model_output_y", yr,"_outliers_10-8-25_reformat_coast.rds"))  %>%
...
k_tau_corr<-k_tau_data_combined_coast %>% 
  dplyr::filter(correlation=="corr") %>% 
  dplyr::select(draw, copula, decade, base_year, dplyr::matches("^pct_.*_(below|above)(10|25|50|75|90)$"), 
                dplyr::matches("^both_(below|above)_")) %>% 
  dplyr::rename_with(~ paste0(., "_corr"), dplyr::starts_with("pct_both_"))%>% 
  dplyr::rename_with(~ paste0(., "_corr"), dplyr::starts_with("both_"))

k_tau_ind<-k_tau_data_combined_coast %>% 
  dplyr::filter(correlation=="ind") %>% 
  dplyr::select(-copula, -correlation) %>% 
  dplyr::rename(copula=copula2) %>% 
  dplyr::select(draw, copula, decade, base_year, dplyr::matches("^pct_.*_(below|above)(10|25|50|75|90)$"), 
                dplyr::matches("^both_(below|above)_")) 

ktau_combine2<-k_tau_corr %>% 
  dplyr::left_join(k_tau_ind, by=c("draw", "copula", "decade","base_year"))

ktau_combine2<-ktau_combine2 %>% 
  dplyr::mutate(pct_diff_num_above10=((both_above_10-both_above_10_corr)/both_above_10_corr)*100, 
                pct_diff_num_below10=((both_below_10-both_below_10_corr)/both_below_10_corr)*100, 
                pct_diff_num_above25=((both_above_25-both_above_25_corr)/both_above_25_corr)*100, 
                pct_diff_num_below25=((both_below_25-both_below_25_corr)/both_below_25_corr)*100, 
                pct_diff_num_above50=((both_above_50-both_above_50_corr)/both_above_50_corr)*100, 
                pct_diff_num_below50=((both_below_50-both_below_50_corr)/both_below_50_corr)*100, 
                pct_diff_num_above75=((both_above_75-both_above_75_corr)/both_above_75_corr)*100, 
                pct_diff_num_below75=((both_below_75-both_below_75_corr)/both_below_75_corr)*100, 
                pct_diff_num_above90=((both_above_90-both_above_90_corr)/both_above_90_corr)*100, 
                pct_diff_num_below90=((both_below_90-both_below_90_corr)/both_below_90_corr)*100)
```
...
### Total CV as % of total value
```{r echo = FALSE, fig.width=10}
ggplot(cv_pct_combined , aes(x = factor(decade), y = cv_pct_total_value, color = factor(copula))) +
  geom_boxplot() +
  labs(x = "Decade", y = "Compensating variation as % of total value")+
  facet_wrap(~ base_year, ncol = 1)
```
...
Projected catch-per-trip data from 2025-09-18.
...
## Coastwide decadal projections - % of trips with catch of both species above/below X percentile  {.tabset}

### above 10th
```{r echo = FALSE, fig.width=10}
ggplot(k_tau_data_combined_coast, aes(x=decade, y=pct_both_above10, color = factor(copula)))+
  geom_boxplot() +
  labs(x = "Decade", y = "%")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  facet_wrap(~ base_year, ncol = 1)
...
### above 25th
```{r echo = FALSE, fig.width=10}
ggplot(k_tau_data_combined_coast, aes(x=decade, y=pct_both_above25, color = factor(copula)))+
  geom_boxplot() +
  labs(x = "Decade", y = "%")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  facet_wrap(~ base_year, ncol = 1)
...
### above 50th
```{r echo = FALSE, fig.width=10}
ggplot(k_tau_data_combined_coast, aes(x=decade, y=pct_both_above50, color = factor(copula)))+
  geom_boxplot() +
  labs(x = "Decade", y = "%")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  facet_wrap(~ base_year, ncol = 1)
...
ggplot(k_tau_data_combined_coast, aes(x=decade, y=pct_both_above75, color = factor(copula)))+
  geom_boxplot() +
  labs(x = "Decade", y = "%")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  facet_wrap(~ base_year, ncol = 1)
...
### above 90th
```{r echo = FALSE, fig.width=8}
ggplot(k_tau_data_combined_coast, aes(x=decade, y=pct_both_above90, color = factor(copula)))+
  geom_boxplot() +
  labs(x = "Decade", y = "%")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  facet_wrap(~ base_year, ncol = 1)
...
### below 10th
```{r echo = FALSE, fig.width=10}
ggplot(k_tau_data_combined_coast, aes(x=decade, y=pct_both_below10, color = factor(copula)))+
  geom_boxplot() +
  labs(x = "Decade", y = "%")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  facet_wrap(~ base_year, ncol = 1)
...
### below 25th
```{r echo = FALSE, fig.width=10}
ggplot(k_tau_data_combined_coast, aes(x=decade, y=pct_both_below25, color = factor(copula)))+
  geom_boxplot() +
  labs(x = "Decade", y = "%")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  facet_wrap(~ base_year, ncol = 1)

# summary_data<- k_tau_data_combined_coast %>% 
#   dplyr::group_by(base_year, correlation) %>% 
#   dplyr::summarise(pct_both_below50=mean(pct_both_below50), 
#                    pct_both_above50=mean(pct_both_above50), 
#                    both_below50=(mean(pct_both_below50)/100)*2000, 
#                    both_above50=(mean(pct_both_above50)/100)*2000) 
#                    
```

### below 50th
```{r echo = FALSE, fig.width=10}
ggplot(k_tau_data_combined_coast, aes(x=decade, y=pct_both_below50, color = factor(copula)))+
  geom_boxplot() +
  labs(x = "Decade", y = "%")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  facet_wrap(~ base_year, ncol = 1)
...
### below 75th
```{r echo = FALSE, fig.width=10}
ggplot(k_tau_data_combined_coast, aes(x=decade, y=pct_both_below75, color = factor(copula)))+
  geom_boxplot() +
  labs(x = "Decade", y = "%")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  facet_wrap(~ base_year, ncol = 1)
...
### below 90th
```{r echo = FALSE, fig.width=10}
ggplot(k_tau_data_combined_coast, aes(x=decade, y=pct_both_below90, color = factor(copula)))+
  geom_boxplot() +
  labs(x = "Decade", y = "%")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  facet_wrap(~ base_year, ncol = 1)
...
### % diff. trips catch above 50th [(ind-corr)/corr]*100
```{r echo = FALSE, fig.width=10}
ggplot(ktau_combine2 , aes(x=decade, y=pct_diff_num_above50, color = factor(copula)))+
  geom_boxplot() +
  labs(x = "Decade", y = "%")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  facet_wrap(~ base_year, ncol = 1)
...
### % diff. trips catch above 75th [(ind-corr)/corr]*100
```{r echo = FALSE, fig.width=10}
ggplot(ktau_combine2 , aes(x=decade, y=pct_diff_num_above75, color = factor(copula)))+
  geom_boxplot() +
  labs(x = "Decade", y = "%")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  facet_wrap(~ base_year, ncol = 1)
...
### % diff. trips catch above 90th [(ind-corr)/corr]*100
```{r echo = FALSE, fig.width=10}
ggplot(ktau_combine2 , aes(x=decade, y=pct_diff_num_above90, color = factor(copula)))+
  geom_boxplot() +
  labs(x = "Decade", y = "%")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  facet_wrap(~ base_year, ncol = 1)
...
### % diff. trips catch below 35th [(ind-corr)/corr]*100
```{r echo = FALSE, fig.width=10}
ggplot(ktau_combine2 , aes(x=decade, y=pct_diff_num_below25, color = factor(copula)))+
  geom_boxplot() +
  labs(x = "Decade", y = "%")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  facet_wrap(~ base_year, ncol = 1)
...
### % diff. trips catch below 50th [(ind-corr)/corr]*100
```{r echo = FALSE, fig.width=10}
ggplot(ktau_combine2 , aes(x=decade, y=pct_diff_num_below50, color = factor(copula)))+
  geom_boxplot() +
  labs(x = "Decade", y = "%")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  facet_wrap(~ base_year, ncol = 1)
...
### % diff. trips catch below 75th [(ind-corr)/corr]*100
```{r echo = FALSE, fig.width=10}
ggplot(ktau_combine2 , aes(x=decade, y=pct_diff_num_below75, color = factor(copula)))+
  geom_boxplot() +
  labs(x = "Decade", y = "%")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  facet_wrap(~ base_year, ncol = 1)
...
for(yr in years){
  #plot data 
  output_summarized1<- read_rds(paste0(output_data_cd,"model_output_y", yr,"_outliers_10-8-25_reformat_coast_mnth.rds"))  %>%
    dplyr::mutate(codreltrip=codrel/ntrips,
                  haddreltrip=haddrel/ntrips, 
                  codrelchoice=codrel/n_choice_occasions,
                  haddrelchoice=haddrel/n_choice_occasions, 
  ...
  diff<-diff_corr %>% 
    ...
    mutate(# Absolute Difference: Difference = Ind - Corr
      diff_cv           = cv_ind - cv_corr,
      diff_codkeep      = codkeep_ind - codkeep_corr,
      diff_haddkeep     = haddkeep_ind - haddkeep_corr,
      diff_codcat       = codcat_ind - codcat_corr,
...
for(yr in years){
  ...
  #plot data 
  output_summarized1<- read_rds(paste0(output_data_cd,"model_output_y",yr,"_outliers_10-8-25_reformat_mode.rds"))  %>%
    dplyr::mutate(codreltrip=codrel/ntrips,
                  haddreltrip=haddrel/ntrips, 
                  codrelchoice=codrel/n_choice_occasions,
                  haddrelchoice=haddrel/n_choice_occasions, 
...
for(yr in years){
  ...
  #plot data 
  output_summarized1<- read_rds(paste0(output_data_cd,"model_output_y", yr,"_outliers_10-8-25_reformat_state.rds"))  %>%
    dplyr::mutate(codreltrip=codrel/ntrips,
                  haddreltrip=haddrel/ntrips, 
                  codrelchoice=codrel/n_choice_occasions,
                  haddrelchoice=haddrel/n_choice_occasions, 
