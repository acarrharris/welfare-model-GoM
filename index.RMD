...
Mean monthly bottom temps by decade, red line indicates annual average. Bars represent mean temp +/- 1.96*standard dev.   
```{r echo = FALSE}
...
ggplot(bt_temp_project, aes(month, average_temp)) + geom_point() +  
...
For reference, here are the number of directed trips by mode and period (1st and 2nd half of month) in 2020 used to calibrate the model. 
...
### MC simulation #2a - annual fishery outcomes with alternative cod open seasons
The baseline open season for cod in 2020 was Sep. 15-30 for private boat and September 7-October 8 for for-hire. Because the model time-step is bi-monthly (two periods per month: 1st half, 2nd half), I set the private open season to the 2nd half of September and the for-hire season to the 1st and 2nd half of September. Haddock was open all months except March and the first half of April (when there was no directed trips). 
...
Main welfare results below show the difference in total CV between baseline cod open season, and cod open season in an alternative month.

## {.tabset}
### Independent (decade=1)
```{r echo = FALSE}
...
Each tab below contains projected fishery outcomes based on input data from alternative copula models. Here, I opened the cod season in different periods across the year.
...
### MC simulation #2b - monthly fishery outcomes with alternative cod open seasons
Each tab contains projected fishery outcomes at the monthly level based on input data from alternative copula models. Here, I opened the cod season in different periods across the year. 
